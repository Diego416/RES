...
namespace SampleMvcApp.Controllers
{
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> SignUp(SignUpViewModel vm, string returnUrl = null)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    {
                        var usuario = new usuarios { UsuarioId = Convert.ToInt32(vm.ID),
                                                    Usu_Nombre = vm.Name,
                                                    Usu_Apellido = vm.LastName,
                                                    Usu_Email = vm.EmailAddress };
